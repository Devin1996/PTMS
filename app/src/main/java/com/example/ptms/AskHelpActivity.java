        ...
        reviewBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(tv_s)) {
                    Toast.makeText(AskHelpActivity.this , "Please Ask your help first" , Toast.LENGTH_SHORT).show();
                } else {
                    addToReviewList(tv_s,  qrValue);
                    Toast.makeText(AskHelpActivity.this , "Your request Submitted Successfully" , Toast.LENGTH_SHORT).show();
                    Intent intentMenu = new Intent(AskHelpActivity.this , PasMenuActivity.class);
...
//        String userPhone = Prevelent.currentOnlineUser.getPhone();
        ...
        reportListRef = FirebaseDatabase.getInstance().getReference();


        reportListRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!(dataSnapshot.child("askForHelp").child(QrValue).child(saveCurrentDate).child(Prevelent.currentOnlineUser.getPhone()).exists())) {
                    ...
                    reportListRef.child("askForHelp").child(QrValue).child(saveCurrentDate).child(Prevelent.currentOnlineUser.getPhone()).updateChildren(reportMap)
                            .addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {

                                    if (task.isSuccessful()) {
                                        Toast.makeText(AskHelpActivity.this , "Your review completed" , Toast.LENGTH_SHORT).show();
                                        loadingBar.dismiss();

                                        Intent intent = new Intent(AskHelpActivity.this , PasMenuActivity.class);
                                        startActivity(intent);
                                        finish();
                                    } else {
                                        loadingBar.dismiss();
                                        Toast.makeText(AskHelpActivity.this , "Network Error.. please try again after some time..." , Toast.LENGTH_SHORT).show();
                                    }
                                }
                            });
                } else {
                    Toast.makeText(AskHelpActivity.this , "your help already exists" , Toast.LENGTH_SHORT).show();
                    loadingBar.dismiss();
